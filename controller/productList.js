        ...
        success: function (data) {
            ...
            if (action) {
                ...
                for (let i = 0; i < jsonL; i++) {
                    if (json[i].id != null) {
                        divCol3 += '<div class="col-3" id="' + json[i].id + '"><div class="card myContainer""><div class="card-body text-center"><input type="checkbox" id="' + json[i].id + '"><p>' + json[i].id + '</p><p>' + json[i].name + '</p><p>' + json[i].price + '</p><p>' + json[i].attributes + '</p><p>' + json[i].value + '</p></div></div></div></div>'
                    } $("#ajtuka").html(divCol3);
                    console.log(divCol3);
                }
            }
